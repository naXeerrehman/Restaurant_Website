...
const editIcon = async (req, res) => {
  try {
    ...
    const updatedIcon = await SocialIcon.findByIdAndUpdate(id, updatedData, {
      new: true,
    });
...
const getIcons = async (req, res) => {
  try {
    const icons = await SocialIcon.find(); // Fetch all icons
...
const deleteIcon = async (req, res) => {
  try {
    ...
    const deletedIcon = await SocialIcon.findByIdAndDelete(id);
